    ...
    public function store(Request $request) {
       ...
       $user = User::find($user_id);
        ...
        $pago = Pago::find($existingPago);
            ...
            $existingProgreso = Progreso::where('user_id', $ultimaVenta->user_id)
                                        ->where('curso_id',$detalleVenta->curso_id)
                                        ->exists();

                                        if(!$existingProgreso){
                                            $progreso = Progreso::create([
                                                'user_id' => $ultimaVenta->user_id,
            ...
            if ($request->tnTipoServicio == 1) {
                ...
                $(document).ready(function() {
                    ...
                    function showModal(mensaje) {
                        const mensaje2  ="Su pedio con MMCA: *'.$lnGuiaCodigo.'* fue cancelado con éxito!, Gracias por elegir nuestro servico.";
